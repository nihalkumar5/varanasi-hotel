        ...
        <div className="min-h-screen bg-[#F7F5F2] pb-32 font-sans text-[#1F1F1F]">
            ...
            {activeOffers.length > 0 && (
                <section className="mt-8 px-4">
                    <div className="bg-white rounded-[32px] border border-black/[0.03] shadow-xl overflow-hidden p-6 relative group">
                        <div className="flex items-center justify-between mb-4 px-1">
                            <div>
                                <p className="text-[10px] font-black text-[#CFA46A] uppercase tracking-[0.3em] mb-1">Curation</p>
                                <h3 className="text-xl font-serif font-black text-[#1F1F1F]">Special Offers</h3>
                            </div>
                            {activeOffers.length > 1 && (
                                <div className="flex gap-2">
                                    <button 
                                        onClick={() => setCurrentOfferIndex(prev => (prev - 1 + activeOffers.length) % activeOffers.length)}
                                        className="w-8 h-8 rounded-full border border-black/5 flex items-center justify-center bg-white hover:bg-slate-50 transition-colors"
                                    >
                        ...
                        <div className="relative h-[220px] rounded-[24px] overflow-hidden bg-slate-100">
                                ...
                                >
                                    ...
                                    <div className="absolute bottom-6 left-6 right-6">
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="px-2 py-0.5 bg-[#CFA46A] text-white text-[8px] font-black uppercase tracking-widest rounded-md">Exclusive</span>
                                            <div className="h-[1px] flex-1 bg-white/20" />
                                        </div>
                                        <h4 className="text-xl font-serif font-black text-white leading-tight mb-2">
                                            {activeOffers[currentOfferIndex].title}
                                        </h4>
                                        <p className="text-[10px] text-white/60 font-medium line-clamp-2 italic">
                                            {activeOffers[currentOfferIndex].description || "Experience the pinnacle of hospitality at Hotel Picasso."}
                                        </p>
                                    </div>
                                    
                                    <div className="absolute top-6 right-6">
                                        <div className="w-10 h-10 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-white/40">
                            ...
                            <div className="flex gap-1.5">
                                {activeOffers.map((_, i) => (
                                    <button
                                        ...
                                        className={`h-1 rounded-full transition-all duration-300 ${i === currentOfferIndex ? 'w-6 bg-[#CFA46A]' : 'w-2 bg-slate-100 hover:bg-slate-200'}`}
                                    />
                                ))}
                            </div>
                            <motion.button
                                whileTap={{ scale: 0.96 }}
                                className="px-5 py-2.5 bg-[#1F1F1F] text-[#CFA46A] rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-black/10 hover:bg-black transition-all"
                            >
